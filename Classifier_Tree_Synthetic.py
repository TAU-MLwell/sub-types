...
def split(node, alpha, left_list, right_list, model_list, records_per_leaf, index, min_size_leaf):
    ...
    if exp <= min_size_leaf and (0.5-alpha) <= error_l <= (0.5+alpha):
        ...
        split(node_left, alpha, left_list, right_list, model_list, records_per_leaf, index_l, min_size_leaf)

    # process right child
    index_r = right_list[index]
    node_right = get_split(right, right_Y)
    error_r = node_left['error']
    exp2 = len(right)
    if exp2 <= min_size_leaf and (0.5-alpha) <= error_r <= (0.5+alpha):
        ...
        right_list.extend([-1,-1])
        model_list.extend([0,0])
        split(node_right, alpha, left_list, right_list, model_list, records_per_leaf, index_r, min_size_leaf)

    return left_list, right_list, model_list, records_per_leaf


# Build a decision tree
def build_tree(features, ground_truth, left_list, right_list, model_list, alpha, min_size_leaf):
    root = get_split(features, ground_truth)
    index = 0
    model_list[index] = root['model']
    records_per_leaf = {}
    lists = split(root, alpha, left_list, right_list, model_list, records_per_leaf, index, min_size_leaf)
    return lists
...
outcomes = build_tree(norm_X, Y, left_list, right_list, model_list, alpha, min_size_leaf)
